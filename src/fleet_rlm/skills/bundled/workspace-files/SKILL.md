...
The root validates their revisions, contents, coverage, and gaps before any
Workspace write or Artifact publication. A child never publishes an Artifact
or updates task memory on its own; only the root's authorized tools and Turn
settlement can do so.

For a sparse lookup, read only the selected path or page. For an exhaustive
inventory, follow each returned cursor and track the files or pages processed.
For a dependent edit, first read and verify the prerequisite content or
checksum, then write with the observed `expected_sha256` when the operation
supports it. Do not report a complete result while required paths or pages are
missing.
...
Session Workspace paths are canonical POSIX-relative paths rooted at `.`. Use the bound tools for this logical namespace; do not guess mounted paths or treat a sandbox-local file as proof of a durable Workspace write. List or inspect before writing, use `overwrite=True` only when replacement is intended, and handle a tool error before trying a different operation. Session Workspace supports full file lifecycle: in addition to write/append, `edit_workspace_text(path, old, new)` replaces one exactly-located fragment (it fails when `old` is absent or ambiguous) and `delete_workspace_path(path)` removes one file or one empty directory (non-empty directories are refused; there is no recursive delete). Both accept an optional `expected_sha256` checksum precondition; use an observed checksum or re-read when guarding against concurrent change.
