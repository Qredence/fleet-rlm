...
def parse_bundled_skill_manifest(directory: Path) -> SkillManifest:
    ...
    disk_resource_paths = {
        ...
        if path.is_file() and path.name != "SKILL.md" and not _is_bytecode_artifact(path)
    }
...
def _is_bytecode_artifact(path: Path) -> bool:
    """Return True for interpreter-generated bytecode, never bundle content.

    ``pip install`` byte-compiles every ``.py`` file in a wheel by default, so
    installed bundles gain ``__pycache__/*.pyc`` siblings that are not declared
    in the manifest. They are deterministic install artifacts rather than
    undeclared resource bodies and must not fail bundle validation.
    """
