...
class LongCoTQASignature(dspy.Signature):
    ...
    reasoning: str = dspy.OutputField(desc="Detailed step-by-step reasoning and chain of thought")
...
class GroundedAnswerWithCitations(dspy.Signature):
    ...
    coverage_notes: str = dspy.OutputField(desc="Notes on coverage gaps or uncertainty in available evidence")
...
class CodeChangePlan(dspy.Signature):
    ...
    validation_commands: list[str] = dspy.OutputField(desc="Commands to verify correctness before completion")
...
class VolumeFileTreeSignature(dspy.Signature):
    ...
    nodes: list[VolumeTreeNode] = dspy.OutputField(desc="Tree nodes with keys path, type, size_bytes, depth")
    ...
    truncated: bool = dspy.OutputField(desc="True when traversal stops early due to node limits")
...
class MemoryActionIntentSignature(dspy.Signature):
    ...
    current_tree: list[VolumeTreeNode] = dspy.InputField(desc="Current memory tree snapshot")
    ...
    action_type: Literal[
        ...
        "migrate",
        "noop",
    ] = dspy.OutputField(desc="Action type: read, write, append, move, delete, mkdir, tree, audit, migrate, noop")
    ...
    risk_level: Literal["low", "medium", "high"] = dspy.OutputField(desc="Risk level: low, medium, high")
    requires_confirmation: bool = dspy.OutputField(desc="Whether explicit confirmation should be required")
...
class MemoryStructureAuditSignature(dspy.Signature):
    ...
    tree_snapshot: list[VolumeTreeNode] = dspy.InputField(desc="Current tree snapshot of the memory directory")
    ...
    recommended_layout: list[str] = dspy.OutputField(desc="Recommended target layout paths and groupings")
    naming_conventions: list[str] = dspy.OutputField(desc="Recommended naming conventions")
    retention_rules: list[str] = dspy.OutputField(desc="Retention/archive policies")
    priority_fixes: list[str] = dspy.OutputField(desc="Highest-priority fixes to apply first")
...
class MemoryStructureMigrationPlanSignature(dspy.Signature):
    ...
    approved_constraints: str = dspy.InputField(desc="Approved constraints for migration operations")
    operations: list[MemoryMigrationOperation] = dspy.OutputField(desc="Operations with keys op, src, dst, reason")
    rollback_steps: list[str] = dspy.OutputField(desc="Rollback steps in reverse operation order")
    verification_checks: list[str] = dspy.OutputField(desc="Post-migration checks to verify integrity")
...
class ReflectAndReviseWorkspaceStep(dspy.Signature):
    ...
    current_plan: str = dspy.InputField(desc="Current execution plan or delegate context summary")
    latest_sandbox_evidence: str = dspy.InputField(
        desc="Selected sandbox evidence snippets or runtime metadata summary"
    )
    latest_tool_or_code_result: str = dspy.InputField(desc="Latest tool, code, or child-result summary")
    loop_state: str = dspy.InputField(desc="Current recursion depth, retry budget, and loop state summary")
    next_action: Literal[
        "recurse",
        "finalize",
        "request_human_review",
        "repair_and_retry",
    ] = dspy.OutputField(desc="Next step: recurse, finalize, request_human_review, or repair_and_retry")
    revised_plan: str = dspy.OutputField(desc="Updated plan for the next recursive step or repair attempt")
...
class AssembleRecursiveWorkspaceContext(dspy.Signature):
    ...
    latest_tool_or_code_result: str = dspy.InputField(desc="Latest tool, code, or child-result summary")
    context_budget: int = dspy.InputField(desc="Maximum target character budget for the next recursive context")
    ...
    selected_evidence_ids: list[str] = dspy.OutputField(desc="Most relevant recent evidence ids to surface next")
    assembled_context_summary: str = dspy.OutputField(
        desc="Bounded summary describing what the next recursive pass should see"
    )
    omission_rationale: str = dspy.OutputField(desc="Why other memory/evidence was omitted to control context size")
...
class PlanRecursiveRepair(dspy.Signature):
    ...
    repair_mode: Literal[
        ...
        "needs_human_review",
        "no_repair",
    ] = dspy.OutputField(desc="Whether to do a targeted repair, bounded repair loop, escalate, or skip repair")
    repair_target: str = dspy.OutputField(desc="The narrow workspace/code/tool target that should be repaired first")
