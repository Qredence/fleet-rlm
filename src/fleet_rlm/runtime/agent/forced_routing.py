...
def run_forced_rlm_turn(
    ...
    *,
    ...
    """Run the explicit sync ReAct→RLM handoff for a forced recursive turn."""
    ...
    return prediction_from_forced_rlm_result(agent, forced_result)
...
async def arun_forced_rlm_turn(
    ...
    *,
    ...
    """Run the explicit async ReAct→RLM handoff for a forced recursive turn."""
    if not message or not message.strip():
        raise ValueError("message cannot be empty")
