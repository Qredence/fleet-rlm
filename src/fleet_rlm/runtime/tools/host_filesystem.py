"""Host filesystem helpers used by legacy read/search tools."""
...
def read_host_file_slice(
    ...
    from fleet_rlm.runtime.content.ingestion import read_document_content as _read_document_content
...
def find_host_files(
    ...
    try:
        ...
        return find_host_files_with_rg_cli(pattern=pattern, path=path, include=include)
...
def find_host_files_with_rg_cli(*, pattern: str, path: str = ".", include: str = "") -> dict[str, Any]:
...
def build_host_filesystem_tools(agent: Any) -> list[Any]:
    """Build host filesystem navigation tools with a shared context object."""
    from dspy import Tool

    ctx = HostFilesystemContext(agent=agent)

    def list_files(path: str = ".", pattern: str = "**/*") -> dict[str, Any]:
        return list_host_files(ctx, path=path, pattern=pattern)
...
__all__ = [
    ...
    "build_host_filesystem_tools",
    "find_host_files",
    "find_host_files_with_rg_cli",
    "list_host_files",
