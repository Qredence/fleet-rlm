...
class RunEventStream(Protocol):
    """Async observation stream owned by the TurnCoordinator."""
...
class OpenedTurnStream:
    """Coordinator-owned stream handle with cancellation-resistant close."""
    ...
    async def wait_open(self, *, timeout: float | None = None) -> OpenedTurnStream | None:
        """Wait for coordinator preparation without cancelling its owned task."""
    ...
    async def aclose(self) -> None:
        """Close once and shield the complete coordinator settlement."""
