    ...
    *,
    ...
    code = "\n".join(
        (
            ...
            "            raise StorageError(errno.EINVAL)",
            "        match = memory_record_re.fullmatch(record)",
            "        if match is None:",
            "            raise StorageError(errno.EINVAL)",
            "        timestamp, category, learning = match.groups()",
            "        try:",
            ...
            "            or len(category) > 64",
            "            or learning != ' '.join(learning.split())",
            "            or '\\x00' in learning",
            "        ):",
            "            raise StorageError(errno.EINVAL)",
            ...
            "    if operation in ('append', 'memory_append'):",
            ...
            "                if operation == 'memory_append' and not stat.S_ISREG(existing_stat.st_mode):",
            "                    fail('unsafe')",
            "                if operation != 'memory_append' and stat.S_ISLNK(existing_stat.st_mode):",
            "                    fail('unsafe')",
            "                if operation != 'memory_append' and stat.S_ISDIR(existing_stat.st_mode):",
            "                    fail('is_directory')",
            "            access_mode = os.O_RDWR if operation == 'memory_append' else os.O_WRONLY",
            "            flags = access_mode | os.O_CREAT | os.O_APPEND | getattr(os, 'O_NOFOLLOW', 0)",
            "            if operation == 'memory_append' and existing_stat is None:",
            "                flags |= os.O_EXCL",
            ...
            "                if not stat.S_ISREG(opened_stat.st_mode):",
            "                    fail('unsafe')",
            "                if existing_stat is not None and (opened_stat.st_dev, opened_stat.st_ino) != (existing_stat.st_dev, existing_stat.st_ino):",  # noqa: E501 - emitted sandbox code is intentionally one line
            "                    fail('unsafe')",
            "            current_size = opened_stat.st_size",
            "            if current_size + len(payload) > max_bytes:",
            "                fail('too_large')",
            "            if operation == 'memory_append' and current_size:",
            "                os.lseek(fd, 0, os.SEEK_SET)",
            "                existing_data = b''",
            "                while len(existing_data) < current_size:",
            "                    chunk = os.read(fd, current_size - len(existing_data))",
            "                    if not chunk:",
            "                        break",
            "                    existing_data += chunk",
            "                if len(existing_data) != current_size:",
            "                    fail('unsafe')",
            "                try:",
            "                    validate_memory_log(existing_data)",
            "                except StorageError:",
            "                    fail('unsafe')",
            "            if operation == 'memory_append' and current_size:",
