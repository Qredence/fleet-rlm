...
worker thread scheduling, integrity guards, and fresh per-Run
`RLMRunner` stream execution.
"""
    ...
    async def _start_worker(
        ...
        observed_tools = tuple(
            observe_tool(
                ...
                is_authorized=lambda: not context.identity.authority.revoked,
                guards=guards,  # ty: ignore[invalid-argument-type]
        ...
        try:
            ...
            worker = start_rlm_worker(
                ...
                kwargs=kwargs,
                ownership=ownership,
                execute=trace.execute,  # ty: ignore[invalid-argument-type]
