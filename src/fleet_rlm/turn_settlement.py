...
@dataclass(frozen=True, slots=True)
class RunSettlementPlan:
    """Immutable dependencies and limits for functional Turn settlement."""
    ...
    promotion: ArtifactPromotion = field(init=False)

    def __post_init__(self) -> None:
        object.__setattr__(self, "promotion", ArtifactPromotion(max_bytes=self.max_artifact_bytes))


def bind_settlement(plan: RunSettlementPlan) -> RunLifecycle:
    """Bind immutable policy inputs to the functions used by one coordinator."""
    return cast(
        RunLifecycle,
        SimpleNamespace(
            begin=partial(begin_run, plan),
            finish=partial(finish_run, plan),
            request_cancel=partial(request_run_cancel, plan),
            heartbeat=partial(heartbeat_run, plan),
            settle=partial(settle_run, plan),
            revoke_claim=partial(revoke_run_claim, plan),
            complete_settling=partial(complete_settling_run, plan),
            heartbeat_seconds=plan.heartbeat_seconds,
            stale_after_seconds=plan.stale_after_seconds,
        ),
    )


async def begin_run(plan: RunSettlementPlan, request: RunClaim) -> RunStart:
    return await plan.store.begin(request)


async def finish_run(
    plan: RunSettlementPlan,
    run: ClaimedRun,
    resolution: RLMOutcome | RunFailure,
    *,
    artifact_sink: RunArtifactSink | None = None,
    ...
    if isinstance(resolution, RLMOutcome):
        resolution = project_outcome_prediction(resolution)
    if isinstance(resolution, RunFailure):
        return await _transition_receipt(plan, run, FailClaim(_claim_failure(resolution), resolution.usage))
    if not resolution.succeeded:
        ...
        return await _transition_receipt(plan, run, FailClaim(_claim_failure(failure), failure.usage))

    candidates = plan.promotion.validate(
    ...
    try:
        validated = await _read_candidates(plan, candidates, artifact_sink)
        stage = "publish_artifacts"
        promoted = await _publish(candidates, validated, artifact_sink, written)
        if run.authority.revoked:
            raise RunLifecycleUnavailableError("Turn claim is no longer available")
        ...
        if run.authority.revoked:
            raise RunLifecycleUnavailableError("Turn claim is no longer available")
        if memory_intents:
            commit_call = plan.store.commit(run, committed, promoted, memory_intents=memory_intents)
        else:
            commit_call = plan.store.commit(run, committed, promoted)
        ...
        if snapshot_path is not None:
            cleanup_cancelled |= await _rollback(result_snapshot_sink, (snapshot_path,))
        cleanup_cancelled |= await _rollback(artifact_sink, reversed(written))
        cleanup_cancelled |= await _rollback(
            artifact_sink,
    ...
    await _reconcile_snapshot_after_commit(snapshot_task, result_snapshot_sink, snapshot_path)
    await _promote_memory_candidates_after_commit(plan, resolution, memory_promotion, run_id=run.run_id)
    await _settle_staging(plan, artifact_sink, candidates)
    return receipt
...
async def _promote_memory_candidates_after_commit(
    ...
    *,
    ...
    outbox = plan.memory_outbox if run_id is not None else None
    with turn_phase_span(
        ...
        if attempt.status == "deadline_exceeded":
            await _mark_run_outbox(run_id, outbox, "note", reason=OUTCOME_DEADLINE_EXCEEDED)
            logger.warning(
        ...
        if attempt.status == "interrupted":
            await _mark_run_outbox(run_id, outbox, "note", reason=OUTCOME_INTERRUPTED)
            logger.warning("Memory Candidate promotion was interrupted after the Turn commit; owned cleanup retained")
            span.set_outputs({"promotion_outcome": "interrupted", "promoted_count": 0})
            return
        if attempt.status == "failed":
            await _mark_run_outbox(run_id, outbox, "note", reason=OUTCOME_PROMOTION_FAILED)
        ...
        if failures:
            ...
            await _mark_run_outbox(run_id, outbox, "complete", reason=OUTCOME_PROMOTED)
...
async def request_run_cancel(plan: RunSettlementPlan, access: TurnAccess, run_id: UUID) -> CancelResult:
    return await plan.store.request_cancel(access, run_id)


async def heartbeat_run(plan: RunSettlementPlan, run: ClaimedRun) -> None:
    await _transition(plan, run, HeartbeatClaim())


async def settle_run(plan: RunSettlementPlan, run: ClaimedRun, failure: RunFailure) -> FailedRunReceipt:
    """Revoke commit authority while retaining the durable claim for cleanup."""
    return await _transition_receipt(plan, run, BeginSettlement(_claim_failure(failure), failure.usage))


async def revoke_run_claim(plan: RunSettlementPlan, run: ClaimedRun, failure: RunFailure) -> FailedRunReceipt:
    """Idempotently classify a Run whose durable claim authority was lost."""
    return await _transition_receipt(plan, run, RevokeClaim(_claim_failure(failure), failure.usage))


async def complete_settling_run(plan: RunSettlementPlan, run: ClaimedRun) -> FailedRunReceipt:
    """Release a retained claim only after owned cleanup has completed."""
    return await _transition_receipt(plan, run, CompleteSettlement())


async def _transition(plan: RunSettlementPlan, run: ClaimedRun, command: ClaimCommand) -> FailedRunReceipt | None:
    return await plan.store.transition_claim(run, command)


async def _transition_receipt(plan: RunSettlementPlan, run: ClaimedRun, command: ClaimCommand) -> FailedRunReceipt:
    with turn_phase_span("Turn.claim_transition", inputs={"command": type(command).__name__}):
        receipt = await _transition(plan, run, command)
    if receipt is None:
...
async def _read_candidates(
    ...
    reads = [
        asyncio.ensure_future(sink.read(candidate.staging_path, max_bytes=plan.max_artifact_bytes))
...
async def _reconcile_snapshot_after_commit(
    ...
    try:
        ...
        if sink is not None and snapshot_path is not None:
            await _rollback(sink, (snapshot_path,))
    else:
