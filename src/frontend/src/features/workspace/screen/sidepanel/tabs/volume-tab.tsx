...
export function VolumeTab({ isMobile }: { isMobile: boolean }) {
  ...
  const { volumes, dataSource, degradedReason, isLoading, isFetching, refetch } = useFilesystem(
    activeProvider,
    { maxDepth: 3, maxEntries: 80 },
  );
  ...
  const treePane = (
    <div data-workspace-volume-tree-panel className="flex h-full min-h-0 w-full flex-col">
  ...
  const previewPane = (
    <div data-workspace-volume-preview-panel className="h-full min-h-0 w-full overflow-hidden">
      {selectedFileNode ? (
  ...
  if (isMobile) {
    return (
      <div data-workspace-volume-layout="stacked" className="flex h-full min-h-0 flex-col">
  ...
  return (
    ...
    >
      <div className="min-w-0 overflow-hidden">{treePane}</div>
      <div
      ...
      />
      <div className="min-w-0 overflow-hidden">{previewPane}</div>
    </div>
  );
}
