...
export function useSessionTraceState() {
  ...
  const tracesQuery = useQuery({
    queryKey: ["workspace", "sidepanel", "session-traces", traceSessionId],
    enabled: Boolean(traceSessionId && hasSessionContent),
    queryFn: ({ signal }) =>
      sessionsEndpoints.traces(String(traceSessionId), { limit: 12, offset: 0 }, signal),
    retry: false,
  });
  ...
  const traceDebugQuery = useQuery({
    ...
    queryFn: ({ signal }) =>
      sessionsEndpoints.traceDebug(
        ...
        { traceId: target.traceId, clientRequestId: target.clientRequestId },
        signal,
      ),
    retry: false,
  });
