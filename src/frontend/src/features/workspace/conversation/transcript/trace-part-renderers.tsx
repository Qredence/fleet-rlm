...
export function WorkspaceToolSessionMessage({ item }: { item: ToolSessionDisplayItem }) {
  const latestItem = item.items[item.items.length - 1];
  const latestState = latestItem ? toolSessionStateForItem(latestItem) : "running";
            ...
            {item.items.map((sessionItem) => (
              ...
              >
                <div className="flex flex-col gap-2 py-0.5">
                  <div className="typo-label-regular text-foreground">
...
export function WorkspaceTracePart({ part, partKey }: { part: ChatRenderPart; partKey: string }) {
  switch (part.kind) {
    case "reasoning":
