...
import { SkillMarkdown } from "@/features/volumes/file-preview/skill-markdown";

describe("SkillMarkdown link sanitization", () => {
  it("renders absolute https links as clickable anchors", () => {
    const html = renderToStaticMarkup(<SkillMarkdown content="[OpenAI](https://openai.com)" />);
  ...
  it("renders malformed links as readable non-clickable text", () => {
    const html = renderToStaticMarkup(<SkillMarkdown content="[Broken](not-a-valid-url)" />);
