...
export const Tool = ({ className, ...props }: ToolProps) => (
  <Collapsible
    className={cn(
      "group not-prose mb-4 w-full rounded-md border border-border-subtle/60",
      className,
    )}
    {...props}
  />
...
// Backward-compatible type alias for custom tool states
...
const statusIcons: Record<ToolPart["state"], ReactNode> = {
  "approval-requested": <ClockIcon className="size-4 text-caution" />,
...
export const ToolInput = ({ className, input, ...props }: ToolInputProps) => (
  ...
  >
    ...
    <div className="rounded-md bg-muted/50">
      <CodeBlock>
        <CodeBlockCode code={JSON.stringify(input, null, 2)} language="json" />
      </CodeBlock>
    </div>
  </div>
);
...
export const ToolOutput = ({
  ...
  if (typeof output === "object" && !isValidElement(output)) {
    ...
    Output = (
      <CodeBlock>
        <CodeBlockCode code={output} language="json" />
      </CodeBlock>
    );
  }
