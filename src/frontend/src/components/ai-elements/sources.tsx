  ...
  return (
    ...
    >
      <span className="inline-flex items-center gap-2 text-[11px] font-medium text-muted-foreground">
        ...
        Sources
        <span className="rounded-full border-subtle px-1.5 py-0 text-[10px]">
          {count}
        </span>
      </span>
      <ChevronDown
...
function Source({
  ...
  if (!hrefSafe) {
    return (
      <div
        data-slot="source"
        className={cn(
          "block rounded-md border-subtle/80 p-2",
          className,
        )}
      >
        {content}
  ...
  return (
    <a
      ...
      className={cn(
        "block rounded-md border-subtle/80 p-2 transition-colors hover:bg-muted/30",
