...
export type CoreMemoryRowProps = {
  part: {
    input?: {
      label?: string;
      value?: string;
...
export const CoreMemoryRow = memo(function CoreMemoryRow({ part }: CoreMemoryRowProps) {
  const label = part.input?.label || "Core memory";
  const value = part.input?.value || "";

  if (!value.trim()) {
    return (
      <TurnInputRowBase icon={<IconBrain className="w-full h-full" />} label={label}>
