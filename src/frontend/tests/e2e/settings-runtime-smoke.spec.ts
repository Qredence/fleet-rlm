...
test("settings renders runtime health and connectivity controls", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Test Modal", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Test LM", exact: true }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByText("Preflight Checks", { exact: true }),
  ).toBeVisible();
