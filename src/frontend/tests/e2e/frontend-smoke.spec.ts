...
test("loads the supported shell surfaces without route crashes", async ({
  page,
}) => {
  await page.goto("/");

  await expect(
    page.getByRole("button", { name: "RLM Workspace", exact: true }),
  ).toBeVisible();
  await expect(
...
test("legacy product routes redirect to the supported surfaces", async ({
  page,
}) => {
...
test("sign-in dialog supports keyboard dismissal and restores focus", async ({
  page,
}) => {
