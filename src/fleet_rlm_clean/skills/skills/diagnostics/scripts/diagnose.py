#!/usr/bin/env python3
"""Offline-friendly diagnostics for fleet_rlm_clean (no secret values printed)."""
...
def check_package() -> bool:
    print("\n--- package ---")
    ok = find_spec("fleet_rlm_clean") is not None
    print_status("fleet_rlm_clean", "OK" if ok else "FAIL")
    return ok
...
def check_env() -> bool:
    ...
    auth = (os.environ.get("FLEET_CLEAN_AUTH_MODE") or "dev").strip().lower()
    auth_ok = auth in {"dev", "neon"}
    print_status("FLEET_CLEAN_AUTH_MODE", auth if auth_ok else f"INVALID ({auth!r})")

    llm_key = os.environ.get("FLEET_CLEAN_LLM_API_KEY", "")
    daytona_key = os.environ.get("FLEET_CLEAN_DAYTONA_API_KEY", "")
    # Presence is informational for offline; live needs these set.
    secret_presence("FLEET_CLEAN_LLM_API_KEY", llm_key)
    secret_presence("FLEET_CLEAN_DAYTONA_API_KEY", daytona_key)

    live = (os.environ.get("FLEET_CLEAN_LIVE_KERNEL") or "false").strip().lower()
    print_status("FLEET_CLEAN_LIVE_KERNEL", live or "false")

    mount = os.environ.get("FLEET_CLEAN_VOLUME_MOUNT_PATH") or "/home/daytona/fleet"
...
def main() -> None:
    print("=" * 40)
    print("fleet_rlm_clean diagnostics")
    print("=" * 40)
