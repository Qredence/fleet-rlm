---
name: rlm
description: "Hub for fleet_rlm_clean: when to use dspy.RLM, turn budgets, progressive skills, and which workflow skill to load next."
---

# fleet_rlm_clean RLM Hub
...
- **Auth:** `FLEET_CLEAN_AUTH_MODE=dev|neon` (fail-closed).
- **Live providers:** opt-in via `FLEET_CLEAN_LIVE_KERNEL` / live settings; offline unit path is default.
