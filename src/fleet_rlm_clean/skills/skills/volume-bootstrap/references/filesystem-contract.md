...
Owned by `fleet_rlm_clean.daytona.paths.VolumePaths`.
...
|-------|----------|-----------------|
| Attachments | `FLEET_CLEAN_UPLOAD_ROOT` on host | Metadata + ids only |
| Artifacts | `FLEET_CLEAN_ARTIFACT_ROOT` on host | Metadata + ids only |
