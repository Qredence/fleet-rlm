...
| `check_codebase_tree.py` | Enforce import boundaries defined in docs/reference/codebase-map.md (runtime/ may not import api.routers, quality/ may only import api.schemas, frontend features/components must use lib/rlm-api for backend types) | None | `uv run python scripts/check_codebase_tree.py` |
...
| `sync_plans_canvas.py` | Regenerate or check the Cursor `plans-roadmap` canvas from canonical phase dossiers | `docs/plan-implementation/phases/*/README.md`; Cursor canvas path for sync/check | `uv run python scripts/sync_plans_canvas.py --check` |
...
| `create_custom_snapshot.py` | Create or explicitly refresh the default Daytona base snapshot with preinstalled Python packages | `DAYTONA_API_KEY`, `DAYTONA_API_URL`, optional `DAYTONA_TARGET` | `uv run python scripts/create_custom_snapshot.py --refresh` |
| `verify_snapshot.py` | Verify a Daytona snapshot by creating a temporary sandbox and importing expected Python packages | `DAYTONA_API_KEY`, `DAYTONA_API_URL`, optional `DAYTONA_TARGET` | `uv run python scripts/verify_snapshot.py --snapshot fleet-rlm-01` |
...
| `validate_rlm_e2e_trace.py` | Run the live websocket tracing harness, or the explicit six-turn direct-RLM promotion gate | Running API server plus DB/auth/runtime env; promotion also needs enabled MLflow and separate legacy/direct servers | `uv run python scripts/validate_rlm_e2e_trace.py --server-url http://127.0.0.1:8000`; promotion: `uv run python scripts/validate_rlm_e2e_trace.py --promotion-gate --legacy-server-url http://127.0.0.1:8000 --direct-server-url http://127.0.0.1:8001` |
...
| `ensure-entrypoint.mjs` | Create `src/frontend/dist/client/index.html` from TanStack Start build metadata when prerendering is disabled for CI/release builds | Existing `src/frontend/dist` build | `node src/frontend/scripts/ensure-entrypoint.mjs` |
