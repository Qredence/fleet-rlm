...
| `mlflow_cli.py` | Export, evaluate, and optimize MLflow-backed datasets | `MLFLOW_TRACKING_URI` and any required MLflow auth | `uv run python scripts/mlflow_cli.py export --output artifacts/mlflow/annotated-traces.json` |
