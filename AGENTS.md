...
- `docs/plan-implementation/README.md` is the canonical backend roadmap interface; phase status, acceptance, and evidence live in its per-phase dossiers. Run `uv run python scripts/sync_plans_canvas.py` and `uv run python scripts/sync_plans_canvas.py --check` after status changes. Phases 3H–5 are complete; Phase 6 is partial with unresolved live evidence.
