...
- `docs/plan-implementation/README.md` is the canonical backend roadmap interface; phase status, acceptance, and evidence live in its per-phase dossiers. Run `uv run python scripts/sync_plans_canvas.py` and `uv run python scripts/sync_plans_canvas.py --check` after status changes. Phases 3H–5 are complete; every Phase 6 acceptance criterion passes in the current working tree, but the phase remains `in_progress_uncommitted` until the changes are committed. Phase 7 typed configuration is next and remains audit-first.
