...
- Never use `DAYTONA_API_KEY` as the `Authorization: Bearer` token for Fleet-RLM API requests (authenticated clients must use Neon JWT); sanitize client-facing prepare/startup errors—never expose raw `str(exc)`, stack traces, credentials, or Daytona/provider internals to API clients.
...
- Chat execution defaults to `legacy_agent_runtime` (`EXECUTION_BACKEND` env; `direct_rlm` is opt-in server-side only and not accepted on `ChatRequest`). Opt-in `direct_rlm` runs real `dspy.RLM` turns via the pooled Daytona interpreter (Phase 2C); Phase 2D (`59b76422`) emits `TURN_INPUTS` and enriches terminal `DONE` with `schema_version` and `history_turns`; Phase 6 adds a correlated post-turn `MLFLOW_SPAN` over FastAPI SSE, while live `TurnProgressRelay` parity remains deferred. Under `execution_mode=auto`, most turns route to direct/tools rather than `dspy.RLM`; use `rlm_only` to force RLM. `POST /api/chat` SSE and WebSocket execution share the same `InterpreterPoolDeps` interpreter-pool lifecycle.
- `docs/plan-implementation/README.md` is the migration-track roadmap (per-phase dossiers); run `uv run python scripts/sync_plans_canvas.py` and `--check` after status changes. The greenfield clean-backend track is `docs/plan-implementation/clean-backend/` (local planning under `.scratch/clean-backend-*`), delivered in parallel package `src/fleet_rlm_clean/` (live `src/fleet_rlm/` untouched until cutover); keep Daytona SDK imports confined to `fleet_rlm_clean.daytona`, and prefer offline-first tests with opt-in live evidence. Root sandbox create defaults `ephemeral=False`; durable attachments/artifacts use Workspace Volume Scope (`VolumeBlobFs`) with path-free public responses; neon auth requires explicit `FLEET_CLEAN_NEON_AUTH_URL` (no silent default) and coarse public auth errors only (`authentication required` / `invalid token` / `authentication unavailable`), with synthetic `X-Fleet-*` identity only in `auth_mode=dev`.
