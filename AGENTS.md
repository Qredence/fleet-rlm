...
- Run the full validation gate (`make format-check`, `make lint`, `make typecheck`, `make test`, `make api-check`, `make check-docs`, `make plans-canvas-check`, `git diff --check`) before commits when the user or phase completion requires it.
...
- `PLANS.md` is the canonical backend roadmap; `make plans-canvas-sync` / `make plans-canvas-check` regenerate the `plans-roadmap` canvas `PHASES` block (re-sync after status changes); do not maintain `PLANS_REORGANIZED.md` as a parallel plan.
...
- Phase 5 tools layer uses policy-aware descriptors/registry in `src/fleet_rlm/tools/` with runtime facades; `discover_tools()` is the single `ToolExposurePolicy` filter and defaults `sandbox_available=False` (sandbox tools opt in explicitly).
