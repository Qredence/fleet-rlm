...
- `src/frontend/src/features/artifacts/components/GraphStepNode.tsx`: added REPL code preview, failed-node error overlay/details, and trajectory Thought/Action/Observation chain rendering.
- `src/frontend/src/features/artifacts/components/graphNodeDetailParsers.ts`: new parser helpers for code/error/trajectory detail extraction and node-state heuristics.
- `src/frontend/src/features/artifacts/components/ArtifactGraph.tsx`: edge elapsed labels plus improved node/error derivation for canvas graph rendering.
- `src/frontend/src/app/pages/skill-creation/backendArtifactEventAdapter.ts`: preserves trajectory payload details for downstream graph TAO parsing.
- `src/frontend/src/features/artifacts/parsers/artifactPayloadSchemas.ts`: new frontend-local Zod schemas for tool/repl/trajectory/final payloads.
- `src/frontend/src/features/artifacts/parsers/artifactPayloadSummaries.ts`: typed contextual summary helpers for timeline and preview.
- `src/frontend/src/features/artifacts/components/ArtifactTimeline.tsx`: typed contextual timeline summaries with safe fallback.
- `src/frontend/src/features/artifacts/components/ArtifactPreview.tsx`: strongly-typed final output rendering branches with error/JSON/text fallback.
- `src/frontend/src/lib/rlm-api/wsClient.ts`: additive optional `analytics_enabled` field on outgoing WS chat messages.
- `src/frontend/src/features/chat/stores/chatStore.ts`: sources telemetry preference and includes it in chat WS requests.
- `src/frontend/src/features/settings/GroupedSettingsPane.tsx`: updated telemetry copy to reflect backend propagation support.
- `src/fleet_rlm/server/schemas.py`: additive optional `analytics_enabled` on WebSocket request schema.
- `src/fleet_rlm/server/routers/ws.py`: per-message runtime telemetry context application around chat execution stream.
- `src/fleet_rlm/analytics/trace_context.py`: runtime telemetry context helpers (`get_runtime_telemetry_enabled`, `runtime_telemetry_enabled_context`).
- `src/fleet_rlm/analytics/posthog_callback.py`: callback tracking enablement now honors runtime telemetry opt-out context.
- `src/fleet_rlm/db/models.py`: model `Index(...)` declarations for Neon-focused tenant/time/status access patterns.
- `migrations/versions/0007_neon_performance_indexes.py`: additive reversible performance index migration for Phase 2+3 schema.
- `tests/unit/test_analytics_callback.py`: runtime telemetry-disabled suppression coverage.
- `src/frontend/src/features/artifacts/components/__tests__/graphNodeDetails.test.tsx`: QRE-304/305/306 graph node detail tests.
- `src/frontend/src/features/artifacts/components/__tests__/artifactGraphEdgeLabels.test.tsx`: QRE-307 edge label tests.
- `src/frontend/src/features/artifacts/components/__tests__/artifactTimelineSummaries.test.tsx`: QRE-309 timeline summary tests.
- `src/frontend/src/features/artifacts/components/__tests__/artifactPreviewTypedOutput.test.tsx`: QRE-310 preview rendering tests.
...
- Import/reference checks:
  ...
  - `rg -n "follow-up ticket|backend AI analytics preference propagation" src/frontend/src` -> ✅ (stale copy removed)
...
- Commands run:
  ...
  - `npx playwright screenshot --device='Desktop Chrome' http://127.0.0.1:8000/ output/playwright/phase-03/home.png`
  - `npx playwright screenshot --device='Desktop Chrome' http://127.0.0.1:8000/settings output/playwright/phase-03/settings.png`
  - `npx playwright screenshot --device='Desktop Chrome' http://127.0.0.1:8000/__dev/chat-elements output/playwright/phase-03/chat-elements.png`
- Flows validated:
  - app shell route renders (`/`)
  - settings route renders (`/settings`)
  - deterministic mock chat route remains healthy (`/__dev/chat-elements`)
- Artifacts:
  - `output/playwright/phase-03/home.png`
  - `output/playwright/phase-03/settings.png`
  - `output/playwright/phase-03/chat-elements.png`
- Failures / retries:
