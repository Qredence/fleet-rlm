...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("status", postgresql.ENUM("pending", "dispatched", "failed", name="outbox_status"), server_default=sa.text("'pending'::outbox_status"), autoincrement=False, nullable=False),
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], name=op.f("outbox_events_tenant_id_fkey"), ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id", name=op.f("outbox_events_pkey")),
    )
    op.create_index(op.f("ix_outbox_events_workspace_created_at"), "outbox_events", ["workspace_id", "created_at"], unique=False)
    op.create_index(op.f("ix_outbox_events_status_available_workspace"), "outbox_events", ["status", "available_at", "workspace_id"], unique=False)
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", sa.UUID(), autoincrement=False, nullable=False),
        sa.Column("workspace_id", sa.UUID(), autoincrement=False, nullable=False),
        sa.Column("provider", postgresql.ENUM("daytona", "aca_jobs", "local", name="sandbox_provider"), autoincrement=False, nullable=False),
        ...
        sa.Column("status", postgresql.ENUM("provisioning", "ready", "error", "archived", name="workspace_volume_status"), server_default=sa.text("'provisioning'::workspace_volume_status"), autoincrement=False, nullable=False),
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], name=op.f("workspace_volumes_tenant_id_fkey"), ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id", name=op.f("workspace_volumes_pkey")),
        sa.UniqueConstraint("workspace_id", "external_volume_id", name=op.f("uq_workspace_volumes_workspace_external_id"), postgresql_include=[], postgresql_nulls_not_distinct=False),
    )
    op.create_index(op.f("ix_workspace_volumes_workspace_updated_at"), "workspace_volumes", ["workspace_id", "updated_at"], unique=False)
    op.create_index(op.f("ix_workspace_volumes_workspace_status"), "workspace_volumes", ["workspace_id", "status"], unique=False)
    ...
    op.create_table(
        ...
        sa.Column("object_type", postgresql.ENUM("file", "directory", name="volume_object_type"), autoincrement=False, nullable=False),
        ...
        sa.ForeignKeyConstraint(["tenant_id"], ["tenants.id"], name=op.f("volume_objects_tenant_id_fkey"), ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["workspace_volume_id"],
        ...
        sa.UniqueConstraint("workspace_volume_id", "path", name=op.f("uq_volume_objects_workspace_volume_path"), postgresql_include=[], postgresql_nulls_not_distinct=False),
    )
    op.create_index(op.f("ix_volume_objects_workspace_volume_modified"), "volume_objects", ["workspace_volume_id", "modified_at"], unique=False)
