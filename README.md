...
`stream_turn()` dispatches through the server-side `ExecutionBackend` seam. The default `legacy_agent_runtime` backend preserves Phase 1 behavior. `direct_rlm` is an opt-in server-side backend (set `EXECUTION_BACKEND=direct_rlm`); it was introduced as a stub in Phase 2A, dispatches to `DirectRLMRunner` as of Phase 2B, runs an opt-in golden path through `dspy.RLM` and the pooled Daytona interpreter as of Phase 2C, and emits `TURN_INPUTS`, trajectory replay events, `TEXT`, structured `ERROR`, and enriched `DONE` metadata as of Phase 2D. `direct_rlm` is not the default and is not exposed on `ChatRequest`. Operators may set `EXECUTION_BACKEND` to the exact value `legacy_agent_runtime` or `direct_rlm`; clients do not send `execution_backend` in `POST /api/chat`.
