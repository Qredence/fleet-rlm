...
Deeper detail: [Product spec](docs/explanation/product-spec.md), [Architecture overview](docs/architecture.md), [Recursive RLM isolation](docs/architecture.md#recursive-rlm-isolation), [Backend codebase map](docs/reference/codebase-map.md), [Frontend/backend integration](docs/reference/frontend-backend-integration.md), [HTTP and WebSocket API](docs/reference/http-api.md).
...
`POST /api/chat` is an AI SDK UIMessage Data Stream Protocol v1 SSE endpoint (`x-vercel-ai-ui-message-stream: v1`, `Content-Type: text/event-stream`). It is mounted at the app root (`/api/chat`, not `/api/v1/chat`) to match the AI SDK `useChat` default path. It accepts a `ChatRequest` body (AI SDK `messages[]` plus optional fleet control fields such as `session_id`, `execution_mode`, `repo_url`, `repo_ref`, `context_paths`, `batch_concurrency`, `docs_path`, `trace`, `trace_mode`, and `selected_skill_ids`) and streams `RuntimeEvent`s as AI SDK v1 stream parts (`text-delta`, `tool-*`, `reasoning`, `finish`, etc.) plus fleet-specific `data-*` custom parts (`data-sandbox-exec`, `data-rlm-delegate`, `data-span`, `data-artifact`, and others), terminated by a `data: [DONE]` line. It reuses the same DSPy runtime (`dspy.RLM`, `AgentRuntime.aiter_chat_turn_stream()`) as the WebSocket endpoint via a transport-neutral `stream_turn()` seam, so it is a transport boundary only, with no behavior change to the WebSocket path. Auth uses a Bearer token via `require_http_identity`; local mode (`auth_required=false`) bypasses auth with an unauthenticated identity. See [ADR-0003](docs/adr/0003-api-chat-ai-sdk-uimessage-stream.md) for the protocol decision and the [backend guide](src/fleet_rlm/AGENTS.md) for the full `RuntimeEventKind` to AI SDK part mapping.

`stream_turn()` dispatches through the server-side `ExecutionBackend` seam. The default `legacy_agent_runtime` backend preserves Phase 1 behavior. `direct_rlm` is promotion-gated and currently an opt-in server-side backend (set `EXECUTION_BACKEND=direct_rlm`); it was introduced as a stub in Phase 2A, dispatches to `DirectRLMRunner` as of Phase 2B, runs an opt-in golden path through `dspy.RLM` and the pooled Daytona interpreter as of Phase 2C, and emits `TURN_INPUTS`, trajectory replay events, `TEXT`, structured `ERROR`, and enriched `DONE` metadata as of Phase 2D. It remains non-default until the configured local promotion matrix passes and is not exposed on `ChatRequest`. Operators may set `EXECUTION_BACKEND` to the exact value `legacy_agent_runtime` or `direct_rlm`; clients do not send `execution_backend` in `POST /api/chat`.
...
Methodology, caveats, and the result breakdown live in [RLM capability evaluation](docs/explanation/rlm-capability-evaluation.md). Local generated result bundles may also exist under `output/`, but the docs page is the stable repository reference.
...
Start with the [documentation home](docs/index.md). Useful entry points:
