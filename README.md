...
`stream_turn()` dispatches through the server-side `ExecutionBackend` seam. The default `legacy_agent_runtime` backend preserves Phase 1 behavior; `direct_rlm` is reserved and currently raises `NotImplementedError("direct_rlm execution backend is not yet implemented")`. Operators may set `EXECUTION_BACKEND` to the exact value `legacy_agent_runtime` or `direct_rlm`; clients do not send `execution_backend` in `POST /api/chat`.
