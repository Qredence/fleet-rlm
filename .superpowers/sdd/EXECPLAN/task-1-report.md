...
Review finding fixed: incomplete `RuntimeInventory` instances are now rejected by `install_runtime_inventory()` before `composition_ready` is set. The regression test proves an incomplete graph leaves `runtime_inventory` unset and readiness false.
...
### Composition lifecycle regression test
