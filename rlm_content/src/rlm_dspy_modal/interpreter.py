...
def _build_default_image(*, python_version: str, pip_packages: Sequence[str]) -> modal.Image:
    return modal.Image.debian_slim(python_version=python_version).pip_install(*pip_packages)
...
class ModalInterpreter:
    ...
    def __init__(
        ...
        *,
        ...
        self.image = image or _build_default_image(python_version=image_python_version, pip_packages=image_pip_packages)
    ...
    def start(self) -> None:
        ...
        self._sandbox = modal.Sandbox.create(app=self.app, image=self.image, secrets=self.secrets)
        self._proc = self._sandbox.exec("python", "-u", "-c", driver_command, bufsize=1, timeout=self.timeout)
    ...
    def _output_names(self) -> list[str]:
        if not self.output_fields:
            return []
        return [field["name"] for field in self.output_fields if isinstance(field, dict) and field.get("name")]
    ...
    def execute(self, code: str, variables: dict[str, Any] | None = None) -> str | FinalOutput:
        ...
        while True:
            try:
                ...
                if stderr_tail:
                    msg += f"\nStderr: {stderr_tail}"
                raise CodeInterpreterError(msg) from exc
