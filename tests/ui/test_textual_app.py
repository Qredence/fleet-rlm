...
class _FakeAgent:
    ...
    def chat_turn(self, message: str):
        ...
        self.history.messages.append(
            {"user_request": message, "assistant_response": output}
        )
        return {
    ...
    def iter_chat_turn_stream(self, message: str, trace: bool, cancel_check=None):
        self.history.messages.append(
            {"user_request": message, "assistant_response": "stream"}
        )
        ...
        yield StreamEvent(
            ...
            payload={
                "trajectory": {
                    "next_thought_0": "I should call fake_tool",
                    "tool_name_0": "fake_tool",
                    "tool_args_0": {"arg": "value"},
                    "observation_0": "Tool returned data",
                    "next_thought_1": "Now I can respond",
                    "tool_name_1": "finish",
                    "tool_args_1": {},
...
@pytest.mark.anyio
async def test_trajectory_routed_to_reasoning_not_transcript():
    """Trajectory thoughts go to reasoning panel, not the main chat."""
    app = CodeChatTextualApp(
        agent=_FakeAgent(),
        config=SessionConfig(profile_name="test", stream=True, trace_mode="compact"),
    )
    async with app.run_test() as pilot:
        ...
        assert app._turn_state is not None
        assert app._turn_state.done is True

        # Trajectory data should NOT be in the main transcript as raw JSON
        transcript_lines = app.query_one("#transcript").lines
        transcript_text = "\n".join(str(line) for line in transcript_lines)
        assert "next_thought_0" not in transcript_text
        assert "observation_0" not in transcript_text


@pytest.mark.anyio
async def test_trajectory_thoughts_appear_in_reasoning_log():
    """Trajectory thoughts/observations are parsed into the reasoning panel."""
    app = CodeChatTextualApp(
        agent=_FakeAgent(),
        config=SessionConfig(profile_name="test", stream=True, trace_mode="compact"),
    )
    async with app.run_test() as pilot:
        ...
        reasoning_lines = app.query_one("#reasoning_log").lines
        reasoning_text = "\n".join(str(line) for line in reasoning_lines)
        # Verify thoughts were routed
        assert "I should call fake_tool" in reasoning_text
        # Verify observations were routed
        assert "Tool returned data" in reasoning_text


@pytest.mark.anyio
async def test_trajectory_tools_appear_in_tools_log():
    """Trajectory tool calls are parsed into the tools panel."""
    app = CodeChatTextualApp(
        agent=_FakeAgent(),
        config=SessionConfig(profile_name="test", stream=True, trace_mode="compact"),
    )
    async with app.run_test() as pilot:
        ...
        # Switch to the Tools tab so the RichLog renders its lines
        tabs = app.query_one("#tabs", TabbedContent)
        tabs.active = "tools-pane"
        await asyncio.sleep(0.05)

        tools_lines = app.query_one("#tools_log").lines
        tools_text = "\n".join(str(line) for line in tools_lines)
        # Verify tool call was routed (finish is excluded)
        assert "fake_tool" in tools_text


@pytest.mark.anyio
async def test_live_stream_cleared_after_turn():
    """Live stream area is cleared once the turn completes."""
    app = CodeChatTextualApp(
        agent=_FakeAgent(),
        config=SessionConfig(profile_name="test", stream=True, trace_mode="compact"),
    )
