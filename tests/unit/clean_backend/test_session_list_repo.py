"""SqlAlchemySessionRepository list/update/archive/turns (offline sqlite)."""
...
async def test_list_filters_by_owner_and_status() -> None:
    ...
    active, total_active = await repo.list(user_id=user, workspace_id=ws, status="active", limit=10, offset=0)
    assert total_active == 1
    assert active[0].title == "Beta notes"

    archived, total_arch = await repo.list(user_id=user, workspace_id=ws, status="archived", limit=10, offset=0)
    assert total_arch == 1
    assert archived[0].id == a.id

    searched, total_s = await repo.list(user_id=user, workspace_id=ws, search="beta", limit=10, offset=0)
    assert total_s == 1
...
async def test_list_turns_paginated() -> None:
    ...
    turns, total = await repo.list_turns(session.id, user_id=user, workspace_id=ws, limit=10, offset=0)
    assert total == 2
