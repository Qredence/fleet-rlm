...
from fleet_rlm.api.auth_errors import PUBLIC_AUTH_DETAIL, AuthError
from fleet_rlm.api.neon_auth import DEFAULT_NEON_AUTH_URL, NeonAuthVerifier
...
async def test_jose_error_maps_to_invalid_kind_without_leaking_exc_text() -> None:
    ...
    with patch("fleet_rlm.api.neon_auth.jwt.decode", side_effect=BadSignatureError("secret-cause")):
        with pytest.raises(AuthError) as excinfo:
...
def test_jwks_past_max_stale_is_unavailable() -> None:
    ...
    with patch("fleet_rlm.api.neon_auth.urllib.request.urlopen", side_effect=TimeoutError("down")):
        with patch("fleet_rlm.api.neon_auth.time.sleep"):
            with pytest.raises(AuthError) as excinfo:
