...
def _provider(monkeypatch: pytest.MonkeyPatch):
    import fleet_rlm.composition.daytona_run_preparation as module
...
@pytest.mark.asyncio
async def test_cancelled_native_context_creation_deletes_late_context_before_root_release(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A native context that outlives its Turn remains fenced until deleted."""
    ...
    class CodeInterpreter:
        async def create_context(self, **_kwargs: Any) -> object:
            context_started.set()
            await release_context.wait()
            return SimpleNamespace(id="late-context")

        async def delete_context(self, context: object, **_kwargs: Any) -> None:
            deleted.append(context)

    sandbox = SimpleNamespace(id="sandbox-1", code_interpreter=CodeInterpreter())
    platform.sandboxes["sandbox-1"] = sandbox
    run = _turn()
    environment = await provider.acquire(run, deadline=float("inf"))
    factory = environment.native_interpreter_factory
    assert factory is not None

    acquisition = asyncio.create_task(factory(deadline=asyncio.get_running_loop().time() + 30))
    await context_started.wait()
    acquisition.cancel()
    with pytest.raises(asyncio.CancelledError):
        await asyncio.wait_for(acquisition, timeout=0.2)

    # Simulate the prepared-turn cleanup callbacks.  Neither the preparation
    # gate nor the resident root may be released while the context request is
    # still unresolved.
    await environment.release()
    if environment.resident_release is not None:
        await environment.resident_release()
    assert manager.released == []
    assert provider.has_pending_acquisitions

    release_context.set()
    while provider._late_native_context_cleanup_tasks or provider._late_native_contexts:
        tasks = tuple(provider._late_native_context_cleanup_tasks.values())
        if tasks:
    ...
    assert getattr(deleted[0], "id", None) == "late-context"
    assert manager.released == [manager.acquired[0]]
    assert provider._resident_root_leases == {}


@pytest.mark.asyncio
async def test_timed_out_native_context_creation_is_retained_for_provider_cleanup(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A provider that ignores its SDK timeout cannot leak an ambiguous root."""
    ...
    class CodeInterpreter:
        async def create_context(self, **_kwargs: Any) -> object:
            context_started.set()
            await release_context.wait()
            return SimpleNamespace(id="timed-out-context")

        async def delete_context(self, context: object, **_kwargs: Any) -> None:
            deleted.append(context)

    platform.sandboxes["sandbox-1"] = SimpleNamespace(id="sandbox-1", code_interpreter=CodeInterpreter())
    environment = await provider.acquire(_turn(), deadline=float("inf"))
    factory = environment.native_interpreter_factory
    assert factory is not None
    acquisition = asyncio.create_task(factory(deadline=asyncio.get_running_loop().time() + 0.05))
    await context_started.wait()
    with pytest.raises(TimeoutError, match="context acquisition timed out"):
        await acquisition
    ...
    assert [getattr(item, "id", None) for item in deleted] == ["timed-out-context"]
    assert manager.released == [manager.acquired[0]]
