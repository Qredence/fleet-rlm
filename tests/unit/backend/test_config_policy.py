...
def test_policy_rejects_stale_revision_and_credential_bearing_database_url(tmp_path: Path) -> None:
    service, _ = _service(tmp_path)
    before = service.read()
    ...
    with pytest.raises(FleetConfigurationError, match="database URL must not embed credentials"):
        service.update(
            scope="defaults",
            path="storage.database_url",
            value="postgresql+asyncpg://user:password@example.test/fleet",
