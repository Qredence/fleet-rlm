...
def test_local_mlflow_server_starts_with_durable_storage_and_stops_owned_process(
    ...
    versions = iter((None, "3.16.0"))
    monkeypatch.setattr(supervisor, "_mlflow_server_version", lambda *_args, **_kwargs: next(versions))
    monkeypatch.setattr(supervisor.importlib.metadata, "version", lambda _name: "3.16.0")
...
def test_local_mlflow_server_reuses_compatible_external_process(
    ...
    monkeypatch.setattr(supervisor, "_mlflow_server_version", lambda *_args, **_kwargs: "3.16.0")
    monkeypatch.setattr(supervisor.importlib.metadata, "version", lambda _name: "3.16.0")
    monkeypatch.setattr(
        supervisor.subprocess,
        "Popen",
...
def test_local_mlflow_server_rejects_incompatible_external_version(
    ...
    with (
        pytest.raises(supervisor.SupervisorError, match=r"reports version 3\.13\.0.*requires 3\.16\.0"),
