...
def _write_archives(directory: Path, *, timestamp: int, reverse: bool) -> None:
...
def test_normalization_makes_archive_bytes_independent_of_source_metadata(tmp_path: Path) -> None:
    ...
    _write_archives(first, timestamp=int(time.time()) - 3600, reverse=False)
    _write_archives(second, timestamp=int(time.time()), reverse=True)
...
def test_normalized_archives_retain_payloads(tmp_path: Path) -> None:
    _write_archives(tmp_path, timestamp=946684800, reverse=False)
