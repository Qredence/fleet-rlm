...
def test_optimize_recursive_repair_module_runs_gepa_and_persists_artifacts(
    ...
    monkeypatch.setattr(
        "fleet_rlm.runtime.quality.optimize_recursive_repair.GEPA",
        _FakeGEPA,
    )
    monkeypatch.setattr(
        "fleet_rlm.runtime.quality.optimize_recursive_repair.dspy.Evaluate",
        _FakeEvaluate,
    )
    ...
    assert result["validation_score"] == 86.0
