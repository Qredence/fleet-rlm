"""Fake Daytona session, runtime, and storage doubles.

Ported from the original tests/unit/fixtures_daytona.py — these are
well-designed fakes that cover the full Daytona integration surface.
"""
...
class FakeDaytonaSession:
    ...
    def matches_current_async_owner(self) -> bool:
        if self.owner_thread_id is None or self.owner_loop_id is None:
            return False
        try:
            return (self.owner_thread_id, self.owner_loop_id) == (
                threading.get_ident(),
