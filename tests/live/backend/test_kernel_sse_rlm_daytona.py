...
- Full RLM: also FLEET_LLM_API_KEY that can reach a chat model
  (set FLEET_LLM_BASE_URL if using an OpenAI-compatible gateway)
"""
...
def _skip_unless_live_daytona() -> None:
    if not _live_enabled():
        pytest.skip("Set FLEET_LIVE=1 for live kernel tests")
        ...
        try:
            ...
            path = _write_evidence(
                "live-kernel-evidence-partial.json",
                {
                    ...
                    "sandbox_id": context.lease.sandbox_id,
                    "volume_id": context.lease.volume_id,
                    "hint": "Set FLEET_LLM_BASE_URL to a working OpenAI-compatible endpoint "
                    "and a valid chat model via FLEET_ROOT_MODEL / FLEET_SUB_MODEL.",
                },
            )
