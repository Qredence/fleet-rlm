...
from fleet_rlm_typescript_sdk import DefaultAioHttpClient, FleetRlmTypescriptSDK, AsyncFleetRlmTypescriptSDK
from fleet_rlm_typescript_sdk._utils import is_dict
...
logging.getLogger("fleet_rlm_typescript_sdk").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[FleetRlmTypescriptSDK]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with FleetRlmTypescriptSDK(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncFleetRlmTypescriptSDK]:
