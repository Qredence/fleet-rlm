...
- **ReAct Orchestrator**: ``dspy.ReAct`` can sit above RLM tools as an interactive
  host-side decision layer (`user_request + history -> assistant_response`).
...
| `chunk_by_timestamps(text, pattern=r"^\d{4}-\d{2}-\d{2}[T ]", flags=re.MULTILINE)` | Split logs by timestamp boundaries. |
| `chunk_by_json_keys(text)`           | Split top-level JSON object entries into per-key chunks.             |
...
### 5. Interactive ReAct + RLM

Useful for conversational orchestration with tool-use transparency.

1.  **Chat turn**: ReAct receives `user_request` and `history`.
2.  **Decide action**: ReAct selects specialized tools (`chunk_host`, `parallel_semantic_map`, `analyze_long_document`, etc.).
3.  **Delegate semantics**: Tool code can use `llm_query()` / `llm_query_batched()` in sandbox.
4.  **Persist state**: Buffers persist in-session; `/data` volume persists across runs.
5.  **Respond**: ReAct returns `assistant_response`, and trajectory can be inspected for debugging.

CLI entrypoint: `uv run fleet-rlm code-chat` (with `run-react-chat` retained as an alias).
