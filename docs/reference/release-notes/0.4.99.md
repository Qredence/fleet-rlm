...
- Reorganized the backend into clearer `runtime/`, `integrations/`, `api/`, and `cli/` ownership boundaries.
- Tightened the shared WebSocket and workbench contract on the Daytona ReAct + DSPy runtime backbone.
...
- Removed browser auth compatibility for the retired `fleet_access_token` localStorage key. The canonical `fleet-rlm:access-token` key is now the supported path.
...
- Added canonical backend package homes for runtime, provider, and terminal responsibilities under `src/fleet_rlm/runtime/*`, `src/fleet_rlm/integrations/*`, and `src/fleet_rlm/cli/terminal/*`.
