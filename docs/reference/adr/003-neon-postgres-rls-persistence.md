...
```python
class Repository:
    async def get_tenant_by_entra_id(self, entra_tenant_id: str) -> Tenant | None: ...
    async def upsert_user(self, tenant_id: uuid.UUID, entra_user_id: str, ...) -> User: ...
    async def create_memory(self, tenant_id: uuid.UUID, ...) -> Memory: ...
```
