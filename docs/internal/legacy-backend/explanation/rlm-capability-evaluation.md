...
## Enterprise 2030 PDF Needle Retrieval Benchmark

Long PDF Q&A benchmark for **precise quote retrieval** (needle-in-haystack) using
`output/the-enterprise-in-2030-report-copy.pdf` (~100k extracted chars).

| Artifact | Purpose |
|---|---|
| `.data/datasets/enterprise-2030-needle-eval.json` | Gold Q&A items (speakers, negatives) |
| `scripts/build_enterprise_2030_gold_set.py` | Regenerate gold from PDF extraction |
| `scripts/evaluate_pdf_needle_retrieval.py` | Automated harness (H1 direct RLM, H2 routing/full agent) |
| `scripts/run_frontend_needle_matrix.py` | Frontend path-detection + routing matrix → `frontend-runs.jsonl` |

**Metrics:** `needle_exact_accuracy`, `routing_accuracy`, `abstention_accuracy`,
`trajectory_grounding_rate`, `followup_regression_rate`.

**Fast routing check (no LLM):**

```bash
uv run python scripts/evaluate_pdf_needle_retrieval.py --routing-only
uv run python scripts/evaluate_pdf_needle_retrieval.py --routing-only --session-context-paths
```

**Frontend matrix (routing preview):**

```bash
uv run python scripts/run_frontend_needle_matrix.py
```

**Success thresholds:** ≥90% exact quotes with path each turn; ≥80% pathless
follow-ups after session context persistence; 100% abstention on negative controls;
≥90% trajectories grounded on `document_text` (no host-path `fitz` access).

See `output/pdf-needle-eval/baseline-report.md` for failure taxonomy and remediation notes.
...
- Historical `scripts/oolong_official_eval.py` (retired at backend cutover) —
