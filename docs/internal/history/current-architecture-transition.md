...
- `agent_host/` and `worker/` are historical terminology for layers consolidated into `api/` and `runtime/`.
- The current ownership story is `api/` -> `runtime/` -> `integrations/daytona/`.
...
4. `src/fleet_rlm/api/routers/ws/stream.py`
5. `src/fleet_rlm/runtime/factory.py`
6. `src/fleet_rlm/runtime/agent/agent.py`
...
- `api/` is transport-thin: auth, websocket lifecycle, runtime services, session/history, event shaping, and hosted policy orchestration.
- `runtime/` and `integrations/daytona/` hold the execution core and sandbox substrate.
