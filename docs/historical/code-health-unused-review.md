...
| `src/fleet_rlm/runtime/memory_tools.py`               | **removed** | —                               | —               | File no longer exists; memory tools moved to `react/tools/memory_intelligence.py`.                                                   |
...
| `src/fleet_rlm/integrations/config/__init__.py`                   | yes         | U2 Likely Unused                | C Investigate   | No static runtime reachability from entry graph; verify dynamic/package-export access before removal.                                |
