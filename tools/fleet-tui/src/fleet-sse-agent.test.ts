...
describe("FleetSseAgent", () => {
  it("forwards the latest user text and exposes UI SSE parts as fullStream", async () => {
    ...
    const agent = new FleetSseAgent(new FleetApiClient({ baseUrl: "http://fleet.test" }), "session-id");
  ...
  it("stops after a server error instead of emitting the following finish:error", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(
        [
    ...
    const result = await agent.stream({ prompt: [{ role: "user", content: "long task" }], options: undefined });

    await expect(collect(result.fullStream)).resolves.toEqual([{ type: "error", error: "Turn timed out" }]);
  });
});
